...
import { BowlingGame } from '../src/bowling'; // Make sure that the path is correct

test('12 strikes should score 300 ', assert => {
...
test('10 pairs of 9 and miss should score 90', assert => {
    const sut = new BowlingGame();
...
test('10 pairs of 5 and spare, with a final 5 should score 150', assert => {
    const sut = new BowlingGame();
 ...
 test('All rolls are 0 should score 0', assert => {
    const sut = new BowlingGame();
    for (let i = 0; i < 20; i++) {
        sut.roll(0);
    }
...
test('A perfect game of strikes should score 300', assert => {
    const sut = new BowlingGame();
    for (let i = 0; i < 12; i++) {
        sut.roll(10);
    }
...
test('Strike in the last frame with two bonus rolls should score correctly', assert => {
    const sut = new BowlingGame();
    for (let i = 0; i < 9; i++) {
        sut.roll(0);
        sut.roll(0);
...
test('Spare in the last frame followed by a bonus roll should score correctly', assert => {
    const sut = new BowlingGame();
    for (let i = 0; i < 9; i++) {
        sut.roll(0);
        sut.roll(0);
